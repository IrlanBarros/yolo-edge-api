...
def parse_args():
    ...
    p.add_argument("--sharpness-threshold", type=float, default=80.0,
                   help="Variância mínima do Laplaciano para aceitar o frame")
    return p.parse_args()
...
def main():
    ...
    try:
        while saved < args.total:
            ...
            if not is_sharp_enough(frame, args.sharpness_threshold):
                skipped += 1
                if args.manual:
